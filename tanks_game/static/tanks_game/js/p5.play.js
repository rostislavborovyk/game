...
function Sprite(pInst, _x, _y, _w, _h) {
  ...
  *
  * In p5.play a Collider is an invisible box or rectangle
  ...
  * 3. setCollider("box")
  * 4. setCollider("box", offsetX, offsetY, radius)
  *
  * @method setCollider
  * @param {String} type Either "rectangle" or "box"
  ...
  */
  this.setCollider = function(type, offsetX, offsetY, width, height) {
    if (!(type === 'rectangle' || type === 'box')) {
      throw new TypeError('setCollider expects the first argument to be either "box" or "rectangle"');
    } else if (type === 'box' && arguments.length > 1 && arguments.length < 4) {
      throw new TypeError('Usage: setCollider("box") or setCollider("box", offsetX, offsetY, radius)');
    } else if (type === 'box' && arguments.length > 4) {
      pInst._warn('Extra parameters to setCollider were ignored. Usage: setCollider("box") or setCollider("box", offsetX, offsetY, radius)');
    ...
    } else if (type === 'box' && arguments.length === 1) {
  ...
  * Will display the sprite as box.
  *
  * @method draw
  */
  this.draw = function()
    ...
    for(var i=0; i<others.length; i++)
      if(this !== others[i] && !this.removed) //you can check collisions within the same group but not on itself
      {
        ...
        if(type === 'overlap') {
            ...
            //if the other is a box I calculate the displacement from here
            if(this.collider instanceof CircleCollider)
...
function CircleCollider(pInst, _center, _radius, _offset) {
  ...
  //should be called only for box vs box
...
function AABB(pInst, _center, _extents, _offset) {
  ...
  this.overlap = function(other)
  {
    ...
    else if(other instanceof CircleCollider)
    {
      ...
      //find closest point to the box on the box
    ...
    else if(other instanceof CircleCollider)
    {
      ...
      if(distance<other.radius)
      {
        ...
        else
        {
          ...
          //project the normal (line between pt and center) onto the box
        }
